...
class LesscssResourceMapper implements GrailsApplicationAware {
  ...
  def map(resource, config) {
    if (grailsApplication.config.grails?.resources?.mappers?.lesscss?.disabled) {
      return
    }

    if (!lessCompiler) {
      lessCompiler = new LessCompiler()
      lessCompiler.compress = grailsApplication.config.grails?.resources?.mappers?.lesscss?.compress == true ?: false
    }
    File originalFile = resource.processedFile
    File input = new File(resource.workDir, resource.sourceUrl);
